...
Мы уже видели [статические переменные внутри функций](../rust-basics/functions.md#static-variables). Глобальные статические переменные отличаются лишь тем, что они доступны не только в пределах одной функции, а из всей программы. Всё остальное — одинаково.
...
Сообщение от компилятора поясняет, что конструировать начальное значение статической переменной можно только при помощи [константной функции](../rust-basics/functions.md#const-functions), коей является `Mutex::new`, но не является `HashMap::new`.
